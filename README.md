...
This deployment is **already connected to a working Cloudinary account** — uploads work out of the box:
...
You don't need to do anything for uploads to work. This is included so you know what powers the upload feature, and in case you ever want to switch to your own Cloudinary account instead.
...
Go to **Settings → Upload → Add upload preset**. Set **Signing Mode** to **Unsigned**, then scroll down and tap **Save**. Afterwards, double-check the preset list actually shows **Unsigned** in the Mode column — Cloudinary can silently save a preset as "Signed" if Save wasn't tapped in the right place, and a signed preset will not work with this static site.

**Step 4 — Paste them into `script.js`**
Open `script.js`. Near the top, replace the `cloudName` and `uploadPreset` values with your own.
...
All of the above works right now on the live site — nothing further to set up.
